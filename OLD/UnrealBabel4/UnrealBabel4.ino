...
//Salida
#define PIN_LED A1 						//Pin Out of Leds Data [D9]
...
void loop() {
	...
	//
	if (wserver.available()) mode = 2;
	else if (Serial.available()) mode = 1;
	...
	if(mode == -1){
		...
		setColor5050_byColor(smode->setStrip(0, now));
		ResetStream();
		SendStream();
	...
	}else if(mode==2){	//Info from PC -> Wifi
